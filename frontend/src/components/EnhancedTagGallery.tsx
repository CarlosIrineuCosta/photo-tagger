...
interface EnhancedTagGalleryProps {
...
export function EnhancedTagGallery({
  items,
  cropMode = false,
  className,
  itemState,
  ...
  const handleTagExclude = useCallback(
    (itemPath: string, tagName: string) => {
      onExcludeTag(itemPath, tagName)
    },
    [onExcludeTag]
  )
            ...
            <CardContent className="flex flex-1 flex-col gap-2.5 p-3">
              ...
              <div className="flex flex-wrap gap-1.5 text-xs">
                {item.display_tags.map((tag) => {
                  const pressed = isLabelApproved(item.path, tag.name)
                  return (
                    <div key={tag.name} className="relative group">
                      <Toggle
                        pressed={pressed}
                        onPressedChange={() => onToggleLabel(item.path, tag)}
                        className={cn(
                      ...
                      <Button
                        ...
                        onClick={() => handleTagExclude(item.path, tag.name)}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                      {tag.original_synonym && (
                        <div className="absolute -bottom-5 left-0 text-[9px] text-muted-foreground whitespace-nowrap">
                          from: {tag.original_synonym}
                        </div>
                      )}
                    </div>
                  )
              ...
              {item.tag_stack.length > 0 && (
                ...
                >
                  {showTagStack[item.path] ? "Hide" : "Show"} backup tags ({item.tag_stack.length})
                </Button>
              )}

              {/* Tag Stack */}
              {showTagStack[item.path] && item.tag_stack.length > 0 && (
                <div className="border-t border-line/30 pt-2">
                  <div className="text-xs text-muted-foreground mb-1">Backup tags:</div>
                  <div className="flex flex-wrap gap-1">
                    {item.tag_stack.slice(0, 5).map((tag) => (
                      <Badge
                        key={tag.name}
                        variant="outline"
                    ...
                    {item.tag_stack.length > 5 && (
                      <Badge variant="outline" className="text-[10px] px-1.5 py-0.5">
                        +{item.tag_stack.length - 5} more
                      </Badge>
                    )}
                  </div>
                </div>
              ...
              {/* Excluded Tags */}
              {item.excluded_tags.length > 0 && (
                <div className="text-xs text-muted-foreground">
                  Excluded: {item.excluded_tags.join(", ")}
                </div>
              )}
            </CardContent>
          </Card>
